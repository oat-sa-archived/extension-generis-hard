...
var returnValue=-1;for(var rowId in this.theData)if(this.theData[rowId].classUri===classUri){returnValue=rowId;break}return returnValue},Switcher.prototype.nextStep=function(){this.currentIndex<this.theData.length?this.decompile===!0&&"decompiled"!==this.theData[this.currentIndex].statusTag?this.decompileClass(this.theData[this.currentIndex].classUri):this.decompile===!1&&"compiled"!==this.theData[this.currentIndex].statusTag&&this.compileClass(this.theData[this.currentIndex].classUri):this.end()},Switcher.prototype.compileClass=function(classUri){var __this=this,rowId=this.getRowIdByUri(classUri);this.setCellData(rowId,"status",__("Switching to Production...")),
$.ajax({type:"POST",url:__this.getActionUrl("compileClass"),data:{classUri:classUri,options:""},dataType:"json",success:function(r){if(__this.addResultData(rowId,r),r.success){var selfCount=r.count,relatedCount=0;for(var relatedClassName in r.relatedClasses)relatedCount+=parseInt(r.relatedClasses[relatedClassName]);var count=" ("+(selfCount+relatedCount)+" "+__("instances")+")";__this.setCellData(rowId,"status",__("Production")+count),__this.setCellData(rowId,"statusTag","compiled")}else __this.setCellData(rowId,"status",__("Failed"));__this.currentIndex++,__this.nextStep()}})},Switcher.prototype.decompileClass=function(classUri){var __this=this,rowId=this.getRowIdByUri(classUri);
this.setCellData(rowId,"status",__("Switching to Design...")),$.ajax({type:"POST",url:__this.getActionUrl("decompileClass"),data:{classUri:classUri,options:""},dataType:"json",success:function(r){if(__this.addResultData(rowId,r),r.success){var selfCount=r.count,relatedCount=0;for(var relatedClassName in r.relatedClasses)relatedCount+=parseInt(r.relatedClasses[relatedClassName]);var count=" ("+(selfCount+relatedCount)+" "+__("instances")+")";__this.setCellData(rowId,"status",__("Design")+count),__this.setCellData(rowId,"statusTag","decompiled")}else __this.setCellData(rowId,"status",__("Failed"));__this.currentIndex++,__this.nextStep()}})},Switcher.prototype.end=function(){
var __this=this;this.options.beforeComplete&&this.options.beforeComplete(this),__this.decompile?__this.options.onCompleteDecompile&&__this.options.onCompleteDecompile(this):$.ajax({type:"POST",url:__this.getActionUrl("createPropertyIndex"),data:{},dataType:"json",success:function(r){__this.options.onComplete&&__this.options.onComplete(this,r.success)}})},Switcher}),define("generisHard/controller/settings/optimizer",["jquery","i18n","generisHard/Switcher"],function($,__,Switcher){return{start:function(){var $compilationGrid=$("#compilation-grid-container"),$compilationResutlts=$("#compilation-grid-results"),$compileButton=$("#compileButton"),$decompileButton=$("#decompileButton"),options={
onStart:function(){$compilationGrid.show()},onStartEmpty:function(){$compilationResutlts.html(__("There are no classes available for optimization.")).show()},onStartDecompile:function(){$compilationGrid.show()},beforeComplete:function(){$compilationResutlts.html(__("Rebuilding indexes, it may take a while.")).show()},onComplete:function(aSwitcher,success){success?$compilationResutlts.html(__("Switch to Production Mode completed.")).show():$compilationResutlts.html(__("Cannot successfully build the optimized table indexes")).show()},onCompleteDecompile:function(){$compilationResutlts.html(__("Switch to Design Mode completed")).show(),$compileButton.show(),$decompileButton.show();
}},mySwitcher=new Switcher("compilation-grid",options);mySwitcher.init(),$compileButton.click(function(){confirm(__("All classes in Design Mode will switch to Production Mode. Please confirm."))&&mySwitcher.startCompilation()}),$decompileButton.click(function(){confirm(__("All classes in Production Mode will switch to Design Mode. Please confirm."))&&(mySwitcher.startDecompilation(),$compilationResutlts.hide())})}}});
